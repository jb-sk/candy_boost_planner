 ...
 *   B) にとよんツール PokemonIv.ts formMap + pokemon.json（英名・番号の正）
...
// にとよん: dexNo → [{ name, form(string), formNum }]
...
// ---------------------------------------------------------------------------
// Phase 1: にとよん formMap の新フォーム名を form-ja-to-number.json / form-label-ja-to-en.json に追加
// ---------------------------------------------------------------------------
...
console.log("--- Phase 1: formMap → マッピングテーブル同期 ---");
...
for (const [enName, nitoyonNum] of Object.entries(nitoyonFormMap)) {
  ...
  if (existingJaNames.length > 0) {
    ...
    if (registeredNums.includes(nitoyonNum)) {
      console.log(`  [OK] "${enName}" = ${nitoyonNum}`);
      continue;
    }
    // 番号が違う → 修正
...
// ---------------------------------------------------------------------------
console.log("--- Phase 2: Wiki/にとよん フォーム番号ずれ → alias 自動追加 ---");
    ...
    for (const ne of nitoyonEntries) {
      ...
      if (formAliases[aliasKey] !== undefined) {
        ...
        if (wikiForms.has(target)) {
          console.log(`  [ALIAS OK] #${dexNo} にとよん form=${ne.formNum} → alias → wiki form=${target}`);
        } else {
          unfixable.push({
      ...
      if (wikiForms.has(0) && ne.formNum !== 0) {
        ...
        console.log(`  [UNFIXABLE] #${dexNo} にとよん form=${ne.formNum} → alias 先を自動決定できない`);
      }
    }
  }
} else {
...
// ---------------------------------------------------------------------------
console.log("--- Phase 3: generate-pokemon-names.mjs formNameToNumber 同期 ---");
...
console.log(`\n[SUMMARY] fixes=${fixes.length} unfixable=${unfixable.length} errors=${finalErrors}`);
