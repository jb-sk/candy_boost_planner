 ...
 * they are a returning user and should not see the onboarding tour.
 */
function hasExistingAppData(): boolean {
  if (typeof localStorage === "undefined") return false;
  for (let i = 0; i < localStorage.length; i++) {
    const key = localStorage.key(i);
    if (key && key.startsWith(APP_PREFIX) && key !== STORAGE_KEY) {
      return true;
    }
  }
  return false;
...
export function useOnboarding() {
  // Treat returning users (who have app data but no onboarding flag) as done
  const alreadyDone = localStorage.getItem(STORAGE_KEY) === "1";
  if (!alreadyDone && hasExistingAppData()) {
    localStorage.setItem(STORAGE_KEY, "1");
  }
  const isDone = ref(alreadyDone || hasExistingAppData());
