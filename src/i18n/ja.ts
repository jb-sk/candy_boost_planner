export const ja = {
  langName: "日本語",
  common: {
    pageNav: "ページ",
    editing: "編集中",
    support: "SUPPORT",
    applyToBox: "ボックスへ反映",
    undo: "元に戻す",
    redo: "やり直す",
    clear: "クリア",
    close: "閉じる",
    load: "読み込む",
    save: "保存",
    delete: "削除",
    optional: "（任意）",
  },
  app: {
    title: "アメブプランナー",
    kicker: "Candy Boost Event Planner",
    lede: "複数ポケモンのアメブースト配分を、総アメ・総かけら制約つきでリアルタイムに計画します。",
  },
  nav: {
    calc: "計算機",
    box: "ポケモンボックス",
  },
  calc: {
    title: "アメブースト計算",
    applyToBoxTitle: "編集内容（現在Lv/あとEXP/EXPタイプ/性格補正）をボックスへ保存",
    applyToBox: "ボックスへ",
    maxShardsLabel: "最大かけら",
    maxShardsHelp: "超過しても計算は継続し、超過分を赤字で表示します。",
    boostKindLabel: "アメブ種別",
    boostKindFull: "アメブ（かけら×{shards} / EXP×{exp}）",
    boostKindMini: "ミニブ（かけら×{shards} / EXP×{exp}）",
    boostKindNone: "通常（かけら×{shards} / EXP×{exp}）",
    boostRemainingLabel: "アメブ残数",
    boostRemainingHelp: "イベント途中の場合、使用可能な残り個数を入力。空欄で上限にリセット。",
    boostRemainingPlaceholder: "{cap}",
    over: "超過",
    remaining: "残り",
    shardsTotal: "かけら合計",
    shardsUnused: "かけら未使用",
    clearPokemons: "ポケモンをクリア",
    shardsUsage: "かけら使用 {pct}%",
    shardsUsageDash: "かけら使用 -",
    boostCandyUsage: "アメブ使用 {pct}%",
    cap: "上限 {cap}",
    selectedUsage: "(選択中のポケモン {pct}%)",
    capUnset: "上限 未設定",
    shardsUsageAria: "かけら使用 {pct}% / 上限 {cap}",
    shardsCapUnsetAria: "かけら上限 未設定",
    boostCandyUsageAria: "アメブ使用 {pct}% / 上限 {cap}",
    slot: "スロット{n}",
    slotEmpty: "空",
    addHint: "追加方法: ポケモンボックスの詳細で「計算する（反映）」を押すと、このリストに追加されます。",
    candyAllocTitle: "万能アメの自動配分について",
    candyAllocDesc: `・タイプアメは積極的に使用（万能Sで端数調整）
・万能アメの必要S量を計算（端数調整で余り0-2/ポケモン）
・Sが十分なら → Sのみ使用（M/Lは使わない）
・Sが不足なら → M→Lの順で使用し、Sで端数調整
※リスト上位のポケモンに優先的に使用
※バッグ圧縮のための参考値として`,
    empty: "まだ計算対象がありません。ポケモンボックスから追加してください。",
    row: {
      dragReorder: "ドラッグして並び替え",
      pickLevelAria: "{label}の選択",
      srcLevel: "現在Lv",
      dstLevel: "目標Lv",
      expRemaining: "あとEXP",
      expLeftNext: "(あとEXP {exp})",
      expType: "EXPタイプ",
      expTypeFixedHint: "EXPタイプは種族固定のため編集できません。",
      nature: "EXP補正（性格）",
      natureNormal: "-",
      natureUp: "▲",
      natureDown: "▼",
      candyTarget: "アメの個数指定",
      candyTargetNone: "未指定",
      reverseCalcMode: "在庫無限",
      reverseCalcModeShort: "在庫∞",
      boostReachLevel: "アメブ目標Lv",
      boostReachLevelNormal: "（無効）",
      boostRatio: "経験値に対するアメブ割合",
      boostRatioNormal: "（無効）",
      boostCandyCount: "アメブ個数",
      boostCandyCountHint: "上げる：個数に応じて目標Lvが上がります<br>下げる：アメブの代わりに通常アメを使います",
      boostCandyCountNormal: "目標EXP調整",
      boostCandyCountNormalHint: "個数を変更すると目標LvとあとEXPが上下します。",
      shards: "かけら",
      candyTotal: "アメ合計",
      breakdown: "内訳",
      breakdownBoost: "アメブ",
      breakdownNormal: "通常アメ",
      candySupply: "アイテム",
      itemUsage: "使用アイテム",
      itemRequired: "必要アイテム",
      candyShortage: "アメ不足",
      boostCandyShortage: "アメブ不足",
      shardsShortage: "かけら不足",
      speciesCandy: "アメ在庫",
      required: "目標まで",
      candyTargetRow: "個数指定",
      used: "到達可能",
      reachedLv: "到達Lv",
      remainingExp: "残EXP",
    },
    candy: {
      universalLabel: "万能アメ在庫",
      universalS: "S",
      universalM: "M",
      universalL: "L",
      typeCandyLabel: "タイプアメ",
      typeCandyToggle: "タイプアメを設定",
      typeS: "S",
      typeM: "M",
      usageLabel: "万能アメ使用",
      usagePct: "{pct}%",
      // 補填表示用
      typeAbbr: "タ",
      uniAbbr: "万",
      shortage: "不足",
      surplus: "余り",
    },
    export: {
      open: "結果を1枚にまとめる",
      saveImage: "画像を保存",
      title: "1枚にまとめる",
      brand: "ポケモンスリープ　育成計画",
      brandBoost: "ポケモンスリープ　アメブースト計画",
      csv: "CSV出力",
      csvDownload: "CSVをダウンロード",
      csvCopy: "CSVをコピー",
      close: "閉じる",
      sheetTitle: "ホリデーアメブ計画シート",
      sumShardsTotal: "かけら合計",
      sumShortage: "アメ不足",
      sumBoostTotal: "アメブ合計",
      sumNormalTotal: "通常アメ合計",
      sumBoostUnused: "アメブ未使用",
      colPokemon: "ポケモン",
      colExpAdj: "EXP補正",
      colNow: "現在",
      colTarget: "目標",
      colBoost: "アメブ",
      colNormal: "通常アメ",
      colTotal: "アメ合計",
      colShards: "かけら",
      colShortage: "アメ不足",
      total: "アメ合計",
      summaryBoost: "合計アメブ",
      summaryShards: "かけら合計",
      summaryCount: "使用ポケモン",
      universalRankingTitle: "万能アメ消費ランキング",
      rankingTotal: "合計",
      labelType: "タイプ",
      labelUni: "万能",
      totalUniversalS: "万能アメS",
      totalUniversalM: "万能アメM",
      totalUniversalL: "万能アメL",
    },
  },
  box: {
    title: "ポケモンボックス",
    clearAll: "ボックス全消去",
    addNew: "新規追加",
    addNewDesc: "「ポケモン名」から図鑑番号を自動判定してフォームを補完します。",
    add: {
      nameDex: "ポケモン名（必須）",
      nameDexPh: "例: ピカチュウ / ピカチュウ (ホリデー)",
      detected: "判定: {name}（#{id} / EXP{expType}）",
      noMatch: "一致なし（手入力扱い）",
      labelOpt: "表記名",
      labelOptPh: "例: ピカチュウA / 推し個体",
      level: "現在Lv",
      nature: "EXP補正（性格）",
      specialtyOpt: "とくい",
      specialtyUnknown: "不明",
      expType: "EXPタイプ",
      expType600Hint: "（標準）",
      ingredientType: "食材（タイプ）",
      ingredientTypeNone: "未選択",
      subSkills: "サブスキル",

      toCalc: "計算する（反映）",
      toBox: "ボックスに追加",
      addToCalc: "計算機に反映",
      addFavorite: "お気に入りに追加",
    },
    import: {
      title: "インポート（にとよん非公式サポート）",
      desc: "1行=1匹（iv または iv＠nickname）。食材/サブスキル等の不要データも含めて raw を保持します。",
      ph: "ここに貼り付け（複数行）",
      paste: "貼り付け",
      fileSelect: "ファイルから読込",
      pasteHelp: "iOSでペーストできない場合: テキストエリアをタップ後、画面下部のキーボード上でペーストするか、ファイルで読み込んでください",
      run: "取り込み",
      addAllFavorite: "すべてお気に入りに追加",
    },
    list: {
      title: "探す",
      searchPh: "検索（ラベル/図鑑番号）",
      clearSearch: "クリア",
      advancedSettings: "詳細設定",
      join: "フィルタ間の結合",
      joinAnd: "AND（両方）",
      joinOr: "OR（どちらか）",
      favorites: "お気に入り",
      favoritesOnlyTitle: "★お気に入りのみ",
      favoritesOnlyAria: "お気に入りのみ",
      specialty: "とくい",
      specialtyAria: "とくい: {name}",
      subskillFilter: "サブスキルで絞り込み",
      subskillJoin: "サブスキル結合",
      subskillJoinOr: "OR（いずれか）",
      subskillJoinAnd: "AND（すべて）",
      subskillClear: "サブスキル選択クリア",
      undoTitle: "直前の変更を取り消し",
      sortKeyAria: "ソート項目",
      sortLabelFav: "表記名（お気に入り優先）",
      sortLevelFav: "レベル（お気に入り優先）",
      sortLabel: "表記名",
      sortLevel: "レベル",
      sortDex: "図鑑番号",
      sortDexFav: "図鑑番号（お気に入り優先）",
      sortAsc: "昇順",
      sortDesc: "降順",
      favorite: "お気に入り",
      selected: "選択中: {name}",
    },
    detail: {
      speciesUnknownHint: "（種族不明のため仮設定）",
      unknownAuto: "不明（自動）",
      unknown: "（不明）",
      ingredients: "食材",
      subSkills: "サブスキル",
      nickname: "ニックネーム",
      nicknameClearHint: "空にすると種族名表示に戻ります",
      speciesLink: "種族（リンク）",
      unlinked: "未リンク",
      dexNo: "（図鑑No.{id}）",
      relinkHint: "（必要なら下で再リンク）",
      relinkPh: "例: ピカチュウ / ピカチュウ (ホリデー)",
      relinkButton: "再リンク",
      relinkCandidate: "候補: {name}（#{id} / EXP{expType}）",
      relinkNoCandidate: "候補なし",
      level: "レベル",
    },
    deleteFromBox: "ボックスから削除",
    empty: "まだ1匹もありません。上でインポートするか、新規追加してください。",
  },
  status: {
    undo: "Undoしました",
    redo: "Redoしました",
    inputEmpty: "入力が空です",
    pasted: "貼り付けました",
    pasteNotAvailable: "貼り付けできませんでした",
    importResult: "取り込み: {added}件 / スキップ: {skipped}件",
    subSkillUnknown: "未知のサブスキルです（補完から選ぶと確実です）",
    subSkillUnknownIgnored: "未知のサブスキルです（保存時は無視されます）",
    nameEmpty: "名前が空です",
    applyCalcToBox: "計算機の値をボックスへ保存しました",
    boxCleared: "ボックスを全消去しました",
    specialtyUpdated: "とくいを更新しました",
    nicknameUpdated: "ニックネームを更新しました",
    favoriteUpdated: "お気に入りを更新しました",
    levelUpdated: "レベルを更新しました",
    expTypeUpdated: "EXPタイプを更新しました",
    natureUpdated: "性格（EXP補正）を更新しました",
    ingredientTypeUpdated: "食材タイプを更新しました",
    subSkillsUpdated: "サブスキルを更新しました",
    exportFailed: "画像の作成に失敗しました（もう一度お試しください）",
    csvDownloaded: "CSVをダウンロードしました",
    csvDownloadFailed: "CSVのダウンロードに失敗しました",
    csvCopied: "CSVをクリップボードにコピーしました",
    csvCopyFailed: "CSVのコピーに失敗しました",
    relinkFailed: "再リンク失敗：名前が一致しません",
    relinkUpdated: "種族リンクを更新しました（図鑑No.{id}）",
    fileLoaded: "ファイルを読み込みました: {name}",
    fileEmpty: "ファイルが空です",
    fileReadError: "ファイルの読み込みに失敗しました",
  },
  confirm: {
    deleteOne: "削除しますか？\n{label}",
    clearBox:
      "ポケモンボックスを全消去しますか？\n登録: {n}匹\n\n※ この操作は、直後であれば「元に戻す」ボタンで取り消せます。",
  },
  help: {
    title: "使い方ガイド",
    basic: {
      title: "基本的な使い方",
      addTitle: "ポケモンを追加",
      addDesc: "新規追加またはインポート（にとよん形式のエクスポートデータ）し、「計算する」ボタンで計算機に追加します。",
      boostKindTitle: "アメブ種別",
      boostKindDesc: "アメブースト/ミニアメブースト/なし から選択します。",
      stockTitle: "在庫設定",
      stockDesc: "アメブの残数、かけら、万能アメ、タイプアメの在庫を入力します。無くても計算できます（超過として表示）。",
      orderTitle: "リストの並び順",
      orderDesc: "計算機のリストの上から順にリソース（万能アメ・かけら）が割り当てられます。名前左の□をドラッグ＆ドロップするか、右の↑↓で優先度を入れ替えられます。",
      usageTitle: "目標設定",
      usageDesc: "「現在Lv」「目標Lv」などを設定。アメブ個数を上げると、獲得経験値を維持するように目標Lvも上がります。",
      resultTitle: "結果の見方",
      resultDesc: "「目標まで」は不足分も含めた必要な全リソース、「到達可能」は現在のリソースで実際に到達できるレベルを示します。",
      summaryTitle: "サマリー（合計）",
      summaryDesc: "画面上部に合計リソース消費量が表示されます。赤字は在庫不足（超過）を表します。",
      exportTitle: "結果を1枚にまとめる",
      exportDesc: "計算結果を画像またはCSVで出力できます。",
      dataTitle: "保存と削除",
      dataDesc: "データはブラウザに自動保存されます。タブ切り替えにより3パターン保存できます。1つ前の操作をやり直せます。",
    },
    purpose: {
      title: "目的別の使い方",
      targetTitle: "目標Lvまでに必要なリソースを知りたい",
      targetDesc: "現在Lv、目標Lv、あとEXPを入力すると、不足分も含めた必要なリソース量が「目標まで」列に表示されます。",
      currentTitle: "現在のリソースで到達できるLvを知りたい",
      currentDesc: "万能アメやかけら、ポケモンのアメ在庫を入力すると、「到達可能」列に結果が出ます。",
      simulateTitle: "アメの数を指定した到達Lvを知りたい",
      simulateDesc: "「アメの個数指定」に使用するアメ数を入れ、「∞（在庫無限）」にチェックをつけると、アイテムやかけらが不足していても到達Lvが確認できます。",
    },
    candy: {
      title: "万能アメの自動配分について",
      item1: "タイプアメは積極的に使用（万能Sで端数調整）",
      item2: "Sが十分なら → Sのみ使用（M/Lは使わない）",
      item3: "Sが不足なら → M→Lの順で使用し、Sで余り2以下に端数調整",
      note1: "※リスト上位のポケモンに優先的に使用",
      note2: "※バッグ圧縮のための参考値として",
    },
    credits: {
      title: "データ出典",
      desc: "本アプリは以下のサイトを参考にしています。",
      nitoyon: "にとよん様 個体値計算機 for ポケモンスリープ",
      raenonx: "RaenonX様 ポケスリ攻略ツール",
      expTable: "経験値テーブル",
      pokemonData: "ポケモンデータ・インポート形式",
    },
  },
} as const;
