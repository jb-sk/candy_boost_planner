 ...
 * アイテム使用量（表示用）
 */
export type DisplayItemUsage = {
  speciesCandy: number;
  typeS: number;
  typeM: number;
  universalS: DisplayValue;  // 補填用なので不足判定あり
  universalM: number;
  universalL: number;
};
...
export type RowResult = {
  // アメブ種別
  boostCandy: DisplayValue;
  normalCandy: DisplayValue;
  candyTotal: DisplayValue;
  shards: DisplayValue;

  // アイテム詳細
  reachableItems: DisplayItemUsage;
 ...
 * 到達可能行の結果（表示用）
 */
export type ReachableRowResult = RowResult & {
  /** 到達Lv */
  reachedLevel: number;
  /** あとEXP（次レベルまでのEXP） */
  expToNextLevel: number;
 ...
 * ポケモンの表示用結果
 */
export type PokemonDisplayResult = {
  // 基本情報
  id: string;
  pokemonName: string;
  pokedexId: number;
  ...
  // 各行
  targetRow: RowResult;              // 目標まで行（常に表示）
  candyTargetRow?: RowResult;         // 個数指定行（candyTargetがある場合のみ）
  reachableRow: ReachableRowResult;  // 到達可能行
};
 ...
 * 表示用の全体結果
 */
export type DisplayPlanResult = {
  /** 表示設定 */
  displayConfig: DisplayConfig;

  /** 各ポケモンの表示用結果 */
  pokemons: PokemonDisplayResult[];
...
export type DisplaySummary = {
  // ─── アメ合計 ───
  /** アメブ合計 */
  boostTotal: DisplayValue;
  /** 通常アメ合計 */
  normalTotal: DisplayValue;
  /** アメ合計（アメブ + 通常） */
  candyTotal: DisplayValue;
  /** かけら合計 */
  shardsTotal: DisplayValue;

  // ─── 超過（使用 - 必要、マイナスなら不足） ───
  /** アメブ超過 */
  boostOverage: number;
  /** かけら超過 */
  shardsOverage: number;
