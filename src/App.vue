...
const calcRowsView = computed(() =>
  calcRows.value.map((r) => {
    ...
    const boxEntry = r.boxId ? box.boxEntries.value.find((x) => x.id === r.boxId) ?? null : null;
    const title = boxEntry ? box.displayBoxTitle(boxEntry) : r.title;
    return {
      ...r,
      title,
...
function applyCalculatorToBox(rowId?: string) {
  ...
  if (!r || !r.boxId) return;
  const e = box.boxEntries.value.find((x) => x.id === r.boxId) ?? null;
  if (!e) return;
  const now = new Date().toISOString();
